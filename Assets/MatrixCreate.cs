...
public class MatrixCreate : MonoBehaviour
{
    ...
    public int visitChk(int x, int y)
    ...
    public int getX()
    { // return current max row size.
        return row;
    }

    public int getY()
    { // return current max col size.
        return col;
    }

    public void setXY(int x, int y)
    { // set current x,y pos setting.
        xPos = x;
        yPos = y;
    }
    ...
    { // set current Pick ColorType status. 
        if (val == -1)
        {
            if (stack.Count > 1)
            {
    ...
    { // get current Pick ColorType status. 
        return getType;
    }

    public void setCurType(int val)
    { // set current ColorType status.
        curType = val;
    }

    public int getCurType()
    { // get current ColorType status.
        return curType;
    }
    public bool isPossibleLink()
    {
        ...
        int type = getGetType();
        var coordinate = new ArrayList() { xPos, yPos};
        exploreTarget.Enqueue(coordinate);
        pass[xPos, yPos] = true;
        while (exploreTarget.Count > 0) {
            ...
            if (xPos - 1 > 0 && pass[xPos -1, yPos] == false) {
                ...
                else if (visit[xPos - 1, yPos] == 0)
                {
                    exploreTarget.Enqueue(new ArrayList() { xPos - 1, yPos });
                }

            }
            if (xPos + 1 < row && pass[xPos + 1, yPos] == false) {
                if (getPos(xPos + 1, yPos) == type)
                {
                    return true;
                }
            ...
            if (yPos - 1 > 0 && pass[xPos, yPos -1] == false) {
                ...
                else if (visit[xPos, yPos - 1] == 0)
                {
                    exploreTarget.Enqueue(new ArrayList() { xPos, yPos - 1 });
                }
            }
            if (yPos + 1 < col && pass[xPos, yPos + 1] == false) {
                if (getPos(xPos, yPos+1) == type)
                {
                    return true;
                }
